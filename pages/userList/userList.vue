					...
					<view class="colonn p-all-30" >
						...
						<block v-for="(item,index) in userList">
							<view class="roww rowsa p-bottom-40">
								...
								<view class="w-208 cebtees" v-if="item.score">{{item.score}}</view>
								<view class="w-208 cebtees" v-else>未打分</view>
								
								<view  class="fw-700 w-208 cebtees" style="color:green;"
								 v-if="item.score">已打分</view>
								 
								<view  class="fw-700 w-208 cebtees" style="color:#E7942E ;"
								@click.stop="markClick(1,item)" v-else>打分</view>
